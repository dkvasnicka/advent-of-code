...
(defparameter series:*suppress-series-warnings* t)

(defun parse-instruction (instr)
  (cons (intern (str:s-first instr))
        (parse-integer (str:s-rest instr))))

(serapeum:defalias pt (serapeum:partial #'make-instance 'point))
...
(defun build-segment (segs instruction)
  (let* ((current-end (end (last-elt segs)))
         (built (make-instance 'line-segment
                               :start current-end
...
(defun read-wirespec (line-string)
  (serapeum:drop 1
    (series:collect-fn 'vector
                       (lambda () (serapeum:vect (make-instance 'line-segment)))
                       #'build-segment
...
(defun xsection-pt (s1 s2)
  (line-segments-intersection-point s1 s2 :exclude-endpoints t))
  ...
  (let ((wire1 (read-wirespec (read-line)))
    ...
    (princ
      (iter outer (for segment in-vector wire2)
            (sum (line-segment-length segment) into travelled1)
            (iter (for l1-segment in-vector wire1)
                  (sum (line-segment-length l1-segment) into travelled2)
                  (in outer
